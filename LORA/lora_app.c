...
void LoRa_Set(void)
{
	...
	sprintf((char*)sendbuf,"AT+WLRATE=%d,%d",LoRa_CFG.chn,LORA_RATE_19K2);//设置信道和空中速率
	lora_send_cmd(sendbuf,"OK",50);
	sprintf((char*)sendbuf,"AT+TPOWER=%d",LORA_PW_20Bbm);//设置发射功率
	lora_send_cmd(sendbuf,"OK",50);
	sprintf((char*)sendbuf,"AT+CWMODE=%d",LORA_MODE_GEN);//设置工作模式
	lora_send_cmd(sendbuf,"OK",50);
	sprintf((char*)sendbuf,"AT+TMODE=%d",LORA_STA_Tran);//设置发送状态
	lora_send_cmd(sendbuf,"OK",50);
	sprintf((char*)sendbuf,"AT+WLTIME=%d",LORA_WLTIME_1S);//设置睡眠时间
	lora_send_cmd(sendbuf,"OK",50);
	sprintf((char*)sendbuf,"AT+UART=%d,%d",LORA_TTLBPS_9600,LORA_TTLPAR_8N1);//设置串口波特率、数据校验位
	lora_send_cmd(sendbuf,"OK",50);
	...
	USART3_RX_STA=0;
	Lora_mode=1;//标记"接收模式"
	usart3_set(LORA_TTLBPS_9600,LORA_TTLPAR_8N1);//返回通信,更新通信串口配置(波特率、数据校验位)
...
{      
	...
	if(LoRa_CFG.mode_sta == LORA_STA_Tran)//透明传输
	{
		...
		sprintf((char*)Tran_Data,"PH_adc=%f\r\n",0.05*(adcx>>4)+0.36);  //PH函数 0.05*data+0.36
...
void LoRa_Process(void)
{
	...
	i2cset();//初始化i2c，only when haiba mode
